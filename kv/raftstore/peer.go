...
func createPeer(storeID uint64, cfg *config.Config, sched chan<- worker.Task,
	...
	tag := fmt.Sprintf("<%d,%d,%d>", region.GetId(), storeID, metaPeer.Id)
	peers := make([]uint64, 0, len(region.Peers))
	for _, peer := range region.Peers {
		peers = append(peers, peer.Id)
	}
	slices.Sort(peers)
	log.Infof("%s peer created start_key:\"%s\" <%v> peers:%v", tag, region.StartKey, region.RegionEpoch, peers)
...
func replicatePeer(storeID uint64, cfg *config.Config, sched chan<- worker.Task,
	...
	tag := fmt.Sprintf("<%d,%d,%d>", regionID, storeID, metaPeer.GetId())
	log.Infof("%s replicate peer", tag)
...
func NewPeer(storeId uint64, cfg *config.Config, engines *engine_util.Engines, region *metapb.Region, regionSched chan<- worker.Task,
	...
	tag := fmt.Sprintf("<%d,%d,%d>", region.GetId(), meta.GetStoreId(), meta.GetId())
