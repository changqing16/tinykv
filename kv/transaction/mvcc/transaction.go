...
func (txn *MvccTxn) GetValue(key []byte) ([]byte, error) {
	...
	iter.Seek(startKey)
	if iter.Valid() && !engine_util.ExceedEndKey(iter.Item().Key(), endKey) {
		writeData, err := iter.Item().ValueCopy(nil)
		iter.Close()
		if err != nil {
			return nil, err
		}
...
func (txn *MvccTxn) CurrentWrite(key []byte) (*Write, uint64, error) {
	...
	for iter.Seek(startKey); iter.Valid(); iter.Next() {
		...
		if write.StartTS == txn.StartTS {
			iter.Close()
			commitTs := decodeTimestamp(dataKey)
