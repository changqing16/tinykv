...
func (c *RaftCluster) processRegionHeartbeat(region *core.RegionInfo) error {
	...
	if region.GetRegionEpoch() == nil {
		return errors.New("no RegionEpoch")
	}
	curRegion := c.GetRegion(region.GetID())
	if curRegion != nil {
		if util.IsEpochStale(region.GetRegionEpoch(), curRegion.GetRegionEpoch()) {
			return errors.New("region is stale")
		}
	} else {
		overlaps := c.ScanRegions(region.GetStartKey(), region.GetEndKey(), -1)
